...
public class CarouselRun {
    ...
    public int next() {
        ...
        int count = 0;
        while (count < DecrementingCarousel.carouselElements.length && DecrementingCarousel.carouselElements[positionArray %= DecrementingCarousel.carouselElements.length] <= 0) {
            positionArray++;
            count++;
        }
        if (count == DecrementingCarousel.carouselElements.length) {
            isRunning = true;
            return -1;
        }
        return DecrementingCarousel.carouselElements[positionArray++]--;
    }
    ...
    public boolean isFinished() {
        for (int var : DecrementingCarousel.carouselElements) {
            if (var > 0) {
                return false;
            }
        }
